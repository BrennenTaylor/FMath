...
public:
   ...
   * @return Vector3
   */
  FMATH_EXPORT friend Vector3 operator*(const Vector3 &lhs,
   ...
   * @return Vector3
   */
  FMATH_EXPORT friend Vector3 operator*(const float lhs,
  ...
  /**
   * @brief Overload streaming operator, convenience for outputing formatted
   * vector
   *
   * @param os
   * @param vec
   * @return std::ostream&
   */
  FMATH_EXPORT friend std::ostream &operator<<(std::ostream &os,
                                               const Vector3 &vec);
  ...
  /**
   * @brief Calculate sqr magnitude of vector, can be used to avoid sqrt in
   * specific cases
   *
   * @return float
   */
  ...
  // Is this helpful?
  static void MakeOrthonormalBasisRHS(Vector3 &v0, Vector3 &v1,
                                      Vector3 &v2) noexcept;
};
} // namespace Farlor
